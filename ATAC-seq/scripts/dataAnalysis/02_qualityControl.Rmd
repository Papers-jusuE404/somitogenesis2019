...
Samples with a score of 2 or higher have around 10-25% of reads in peaks.
...
Another measure of the quality of an ATAC-seq library is the degree of enrichment of reads around the transcription start sites (TSSs) of genes. To check this, we use the RNA-seq data to retain the genes expressed at moderate to high levels (8,749 genes) and extract the coordinate of their most 5' TSS.

```{r tss}
## BAM files are first converted to BEDPE format
## BEDPE files are shifted to account for the 9bp signature of the transposase
# perl shiftAlns_fromBEDPE.pl -dir BWA/
## BEDPE files are converted back to BAM, sorted and indexed

## For consistency, to get the coordinates of the TSSs we will use ensembl v93 annotation (same as RNAseq).
## We downloaded all TSS coordinates from biomart and used a perl script to retain the earliest TSS for each gene
# in /Users/ibarra01/OneDrive/OneDrive - Cancer Research UK, Cambridge Institute/SOMITES/ATACseq/TSSenrichment: perl getEarliestTSS.pl -file TSScoordinates_ensembl93.tab > TSScoordinates_ensembl93_earliest.tab
genes <- read.table(paste0(dir, "data/ATAC-seq/TSScoordinates_ensembl93_earliest.tab"), sep="\t", row.names = 1, stringsAsFactors = FALSE)
colnames(genes) <- c("gene", "chr", "start", "end", "strand", "TSS")

## to calculate TSS enrichment scores we will restrict to genes that are expressed in the RNAseq data.
expr <- read.table(paste0(dir, "data/RNA-seq/geneCounts_SOMITES.RAW.tab"))[-c(1:4),]
...
genes <- genes[names(means[keep]),]
genes <- GRanges(seqnames = paste0("chr",genes$chr), IRanges(genes$TSS-1000, genes$TSS+1000), names = row.names(genes), gene=genes$gene)
genes <- subset(genes, seqnames(genes) %in% paste0("chr",c(1:19,"X")))
seqlevels(genes) <- seqlevelsInUse(genes)
```

We then create intervals of 2kb centered at each TSS and count the number of insertions occurring in each base, for each gene, in each sample. An insertion is regarded as the 5' and 3' ends of a paired-end fragment, after shifting the coordinates to account for the 9bp used by Tn5 for the insertion.

```{r countInsertions, eval=FALSE}
## to speed up the process we do this as batch scripts, splitting samples into 13 groups of 6
bam.files <- paste0(substr(bam.files, 1, nchar(bam.files)-4), ".shifted.bam")
views <- BamViews(bamPaths = paste0(dir, bam.files), bamIndicies = paste0(dir, bam.files), bamRanges = genes)

params <- ScanBamParam(what=c("flag"))
reads <- suppressWarnings(readGAlignments(views, param = params))

countInsertions <- function(gene, reads){
  overlaps <- reads[subjectHits(findOverlaps(gene, reads))]
  positions <- table(c(start(overlaps[mcols(overlaps)$flag %in% c(99,161),])-start(gene)+1,end(overlaps[mcols(overlaps)$flag %in% c(147,81),])-start(gene)+1))
  positions <- positions[names(positions) %in% 1:2001]
  tmp <- vector(mode="numeric", length = 2001)
  names(tmp) <- 1:2001
  tmp[names(positions)] <- positions
  return(tmp)
}

tss.enrich <- list()
for(i in 1:length(reads)){
  sample <- substr(names(reads), 1, nchar(names(reads))-22)[i]
  tss.enrich[[sample]] <- t(sapply(1:length(genes), function(x) countInsertions(gene=genes[x], reads=reads[[i]])))
  rownames(tss.enrich[[sample]]) <- genes$gene
}
# saveRDS(tss.enrich, paste0(dir, "results/ATAC-seq/01_TSSenrichmentMatrix.Rds"))
```

The aggregate profile for all genes gives us a measure of the number of insertions at each basepair, relative to the TSS. We use the first and last 100bp of the 2kb interval as a measurement of the background insertion level, and compute the fold-change of each bp against this. Thus, the flanks should start at 1 and the fold-change should increase if there is an excess of reads as we approach the TSS. 
