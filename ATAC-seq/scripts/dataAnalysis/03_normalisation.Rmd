...
```{r}
plot(df$PC1, df$readsInPeakSet/df$goodQuality*100)
```
...
## SVAseq
svobj.seq = svaseq(norm.counts, design, mod0, n.sv = n.sv)
```


```{r MDScorrected, fig.width=10, fig.height=4}
norm.counts.corr.seq <- removeBatchEffect(re.adjc, covariates = svobj.seq$sv)

# mds.norm.corr <- plotMDS(norm.counts.corr, top=5000, plot=FALSE)
vars <- rowVars(norm.counts.corr.seq)
tmp <- norm.counts.corr.seq[order(vars, decreasing=TRUE)[1:5000],]
pca <- prcomp(t(tmp))
df <- cbind(pca$x, meta)
