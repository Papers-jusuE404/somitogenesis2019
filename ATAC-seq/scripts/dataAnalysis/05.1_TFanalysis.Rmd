...
plots[[2]] <- ggplot(go.hoxCorr.stats, aes(pos+neg, corrPos, label = term)) + geom_point() + ylab("") + xlab("total correlated pairs in category") + geom_hline(yintercept = 0.5, lty=2, col="red") + geom_text_repel(data = rbind(go.hoxCorr.stats[c(12,21,57,54,62,23,83,113,120,34,134,136),], subset(go.hoxCorr.stats, go.hoxCorr.stats$corrPos < 0.5)), nudge_x = 70, direction = "y", angle = 0, segment.size = 0.25)
multiplot(plotlist = plots, cols=2)
```
...
```{r nHoxcorr}
plot(colSums(go.hoxCorr.stats[,1:37]))
```
