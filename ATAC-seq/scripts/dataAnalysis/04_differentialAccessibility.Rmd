...
* PCA on the residuals of the model containing the somite-stage information: the first few PCs are correlated to FRiP and coniseration of the first 18 components cleans up the data nicely.

### Differential accessibility analysis

Now we can perform differential analysis to obtain differences between the somite trios, and between stages, using the normalised data while regressing out either the SV or PCs that capture the remainjing technical variation in the data.

#### SVA approach

First, let's just use the one SV identified by `sva` as a covariate in the design matrix. We use `edgeR` to perform the differential testing. We first estimate the dispersion and fit the design containing the interaction of somite and stage, plus the SV from sva.
...
saveRDS(y, file=paste0(dir, "ATAC-seq/results/04_edgeRobject_sva.Rds"))
```
...
saveRDS(somiteTrios.perStage.sva, paste0(dir, "ATAC-seq/results/04_diffAccessibility_somiteTrios_perStage_sva.Rds"))
...
df <- t(data.frame(stage8=sum(mcols(somiteTrios.perStage.sva[['stage8']])$FDR <= 0.05),
           ...
           stage27=sum(mcols(somiteTrios.perStage.sva[['stage27']])$FDR <= 0.05),
...
saveRDS(somiteTrios.average.sva, paste0(dir, "ATAC-seq/results/04_diffAccessibility_somiteTrios_average_sva.Rds"))
...
Again, the numbers are low, but a few differences are picked up.
...
We test all pairwise comparisons between stages, taking the average of somites I, II and III.
...
rowData(filtered.data) <- cbind(rowData(filtered.data), test$table)
saveRDS(filtered.data, paste0(dir, "DATAanalysis/filteredCountMatrix_withResults_csaw_svaTrend.Rds"))
```
...
```{r merge}
merged <- mergeWindows(rowRanges(filtered.data), tol=300L)
tabcom <- combineTests(merged$id, test$table)
...
newPeaks <- merged$region
mcols(newPeaks) <- tabcom
saveRDS(newPeaks, paste0(dir, "DATAanalysis/peakCalls_allSamplesTogether_DAresults_svaTrend.Rds"))

sig <- newPeaks[newPeaks$FDR<0.05,]
saveRDS(sig, file=paste0(dir, "DATAanalysis/significantPeaks_acrossStages.Rds"))
```

We get a little over 6 thousand of such regions. We can now explore these results with the shiny app.
