...
```{r examples, fig.width=12, fig.height=10, eval=FALSE, echo=FALSE}
## select the genes with largest fold-changes
tmp <- side.SII[order(abs(side.SII$logFC), decreasing = TRUE),]
genes <- as.character(tmp[tmp$logCPM>0,]$genes[1:9])

plots <- list()
for(g in genes){
  df <- data.frame(sample=colnames(dataNorm)[-1], expr=as.numeric(dataNorm[dataNorm$gene==g,-1]),
                 embryo=meta$embryo, somite=meta$somite, side=ifelse(meta$side==1, "left","right"))
  plots[[g]] <- ggplot(df, aes(as.factor(side), expr, col=meta$embryo, shape=meta$somite)) + geom_point() + ggtitle(g) + labs(col="embryo", shape="somite") + xlab("side of somite")
}
