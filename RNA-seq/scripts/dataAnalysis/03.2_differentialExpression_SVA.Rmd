...
```{r ivsii_only, fig.width=9, fig.height=6}
...
```{r s18.sp, , fig.width=9, fig.height=3}
plots <- list()
for(i in 1:3){
...
In all, we get a little over a thousand (`r sum(length(union(somiteIIvsI.all[somiteIIvsI.all$FDR<0.05,1], somiteIIIvsI.all[somiteIIIvsI.all$FDR<0.05,1])), length(s8.specific), length(s18.specific), length(s35.specific))`) genes differentially expressed between somites, with the vast majority of these showing incremental changes as differentiation proceeds, and a concerted behaviour across development.
